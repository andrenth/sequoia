...
module type POSTGRESQL_NULL_FIELD = sig
  include NULL_FIELD
...
module type POSTGRESQL_FIELD = sig
  include FIELD
...
module type POSTGRESQL_TABLE = sig
  type t
  val table : t Table.t

  module Field : sig
    include POSTGRESQL_FIELD with type table = t
    module Null : POSTGRESQL_NULL_FIELD with type 'a t := 'a t
  end
end

module MakePostgresqlTable (T: Sequoia.NAMED) : POSTGRESQL_TABLE = struct
...
let table name =
  (module struct
    include MakePostgresqlTable (struct type t let name = name end)
  end : POSTGRESQL_TABLE)
